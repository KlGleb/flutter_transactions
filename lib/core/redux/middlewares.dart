...
Middleware<AppState> _createLogout(AuthRepository repository) =>
        (Store<AppState> store, action, NextDispatcher next) {
      next(action);
      repository.logOut().then((value) => store.dispatch(LogoutSuccess()));
    };

Middleware<AppState> _createLogin(AuthRepository repository) =>
        (Store<AppState> store, action, NextDispatcher next) {
      next(action);
      repository
          .logIn(action.userName, action.password)
          .then((_) => store.dispatch(LoginSuccess()));
    };
